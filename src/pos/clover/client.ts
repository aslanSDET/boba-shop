 ...
 *
 * These endpoints live under `app/api/clover/` and are reachable in any build,
 * but they hold one merchant's credentials. On an Asian Kitchen deployment they
 * are not "not yet wired" — they are somebody else's, and answering would be
 * wrong. Routes call this first and return its response when it is not null.
 */
export function notThisDeployment(): Response | null {
  if (ACTIVE_RESTAURANT === "snowdaes") return null;
  return Response.json(
    { error: "This deployment does not use Clover." },
    { status: 404 },
  );
}
